...
<template>
  <main class="app-shell">
    ...
    <section v-else-if="screen === 'game'" class="game-screen">
      ...
      <section class="avatar-zone"><div class="body-controls" aria-hidden="true">頭<br>頸<br>身<br>褲<br>膝<br>腳</div><div class="avatar"><img class="base-body" :class="{ 'body-without-head': activeHeadAsset }" :src="assetUrl(activeHeadAsset ? 'body.png' : 'body_full.png')" alt="阿梅角色底圖"><img v-if="activeHeadAsset" class="base-head" :src="assetUrl(activeHeadAsset)" alt="阿梅帽子專用頭部"><template v-for="item in wornItems" :key="item!.id"><div v-for="layer in item!.wearLayers" :key="layer" class="garment-layer" :class="`layer-${item!.slot}`" :style="garmentStyle(item!, layer)"><span class="garment-fill"></span><img class="garment-outline" :src="assetUrl(layer)" alt=""></div></template></div></section>
